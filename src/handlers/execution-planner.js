...
function addStep(planId, stepConfig = {}) {
  ...
  const step = {
    ...
    action_type: stepConfig.action_type || ACTION_TYPE.SAFE_AUTONOMOUS,
    
    // Dependencies
    dependencies: stepConfig.dependencies || [],
    
    // Approval
    // Default: SAFE_AUTONOMOUS doesn't need approval, others do (unless explicitly set)
    approval_required: stepConfig.approval_required === true || 
                        (stepConfig.approval_required !== false && 
                         stepConfig.action_type !== ACTION_TYPE.SAFE_AUTONOMOUS &&
                         stepConfig.action_type !== undefined),
    approval_token: null,
    approved_at: null,
    approved_by: null,
