...
echo "=== STEP 3: Creating fresh task (with goal field) ==="
FRESH_TASK=$(curl -s -X POST "${SUPABASE_URL}/rest/v1/tasks" \
  ...
  -d '{
    "status": "created",
    "goal": "Route test task for moosa-worker",
...
# Try without action_type since it doesn't seem to exist in dispatches
