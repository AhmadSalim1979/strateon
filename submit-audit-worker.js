/**
 * Qiyadon Pipeline Leak Audit — Form Handler Worker
 * Receives form submissions and emails them to contact@qiyadon.com
 */
  ...
  <div class="container">
    ...
    <div class="body">
      <div class="field">
        ...
        <div class="field-value">${data.phone ? escapeHtml(data.phone) : '<em>Not provided</em>'}</div>
      </div>
      <div class="field">
        <div class="field-label">Industry</div>
        <div class="field-value">${escapeHtml(data.industry)}</div>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="padding-right:16px;">
            <div class="field-label">Leads / Month</div>
            <div class="field-value">${escapeHtml(data.leads_per_month)}</div>
          </td>
          <td>
            <div class="field-label">Close Rate</div>
            <div class="field-value">${escapeHtml(data.close_rate)}</div>
          </td>
        </tr>
      </table>
      <div class="field" style="margin-top:20px;">
        <div class="field-label">CRM / Pipeline Tool</div>
        <div class="field-value">${escapeHtml(data.crm)}</div>
      </div>
      <hr class="divider">
      <div class="field">
        <div class="field-label">Biggest Pipeline Frustration</div>
        <div class="field-value textarea">${escapeHtml(data.challenge)}</div>
      </div>
      <div class="field">
        <div class="field-label">Current Follow-Up Process</div>
        <div class="field-value">${escapeHtml(data.followup_process)}</div>
      </div>
    ...
    <div class="footer">
      Received: ${data.submitted_at ? new Date(data.submitted_at).toLocaleString('Europe/Berlin', {timeZone: 'Europe/Berlin'}) : 'just now'} (Berlin time) · Qiyadon Pipeline Execution
    </div>
  </div>
</body>
</html>
