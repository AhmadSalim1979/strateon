...
exports.saveDriftReport = saveDriftReport;
exports.loadDriftReports = loadDriftReports;
exports.getDriftStatistics = getDriftStatistics;
 ...
 */
function detectDrift(n8nExecution, orchestrationExecution, options) {
    const report_id = `drift-${Date.now()}-${(0, uuid_1.v4)().slice(0, 8)}`;
    const compared_at = new Date().toISOString();
    console.log(`[drift-detector] Comparing N8N (${n8nExecution.length} nodes) vs ` +
    ...
    const maxSeverity = drift_points.reduce((max, dp) => (severityScores[dp.severity] > severityScores[max] ? dp.severity : max), 'none');
    ...
    console.log(`[drift-detector] Report ${report_id}: ${drift_points.length} drift points, ` +
        `severity=${maxSeverity}, drift_detected=${report.drift_detected}`);
    return report;
}
function compareOutputs(n8nOutput, orchOutput) {
...
function findObjectDiff(n8n, orch) {
...
// ─── Ordering Drift ─────────────────────────────────────────────────────────────
function checkOrderingDrift(n8nExecution, orchExecution) {
    ...
    for (let i = 0; i < commonNodes.length - 1; i++) {
        for (let j = i + 1; j < commonNodes.length - 1; j++) {
            ...
            if ((n8nIdx < n8nJdx && orchIdx > orchJdx) ||
...
// ─── Timing Drift ──────────────────────────────────────────────────────────────
function checkTimingDrift(n8nExecution, orchExecution) {
    const n8nTotal = n8nExecution.reduce((sum, n) => sum + (n.duration_ms || 0), 0);
    const orchTotal = orchExecution.reduce((sum, n) => sum + (n.duration_ms || 0), 0);
    if (n8nTotal === 0)
        return null;
...
function computeExecutionComparison(n8nExecution, orchExecution) {
    const n8nDuration = n8nExecution.reduce((sum, n) => sum + (n.duration_ms || 0), 0);
    const orchDuration = orchExecution.reduce((sum, n) => sum + (n.duration_ms || 0), 0);
...
// ─── Payload Similarity ────────────────────────────────────────────────────────
function computePayloadSimilarity(a, b) {
...
function generateRecommendation(severity, driftCount, comparison) {
    if (driftCount === 0) {
 ...
 */
async function saveDriftReport(report) {
 ...
 */
async function loadDriftReports(shadow_run_id) {
    const { getClient } = await Promise.resolve().then(() => __importStar(require('../persistence/supabase-client')));
    try {
        const supabase = getClient();
 ...
 */
async function getDriftStatistics() {
    const { getClient } = await Promise.resolve().then(() => __importStar(require('../persistence/supabase-client')));
    try {
        const supabase = getClient();
        const { data, error } = await supabase
