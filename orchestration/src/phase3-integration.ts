...
import { initializeWebhookHandlers, verifyIntegration, shutdownPhase3 } from './phase3-webhook-integration';
import { startQueueMonitor, stopQueueMonitor } from './monitoring/queue-monitor';
...
async function main() {
  ...
  const verification = await verifyIntegration();
  ...
  const shutdownHandlers = await initializeWebhookHandlers();
  ...
  statusApp.get('/status', async (_req: any, res: any) => {
    const stats = getQueueStats();
    const verification2 = await verifyIntegration();
