...
import { validateRuntimeConfig } from './config-validator.js';
import { isShadowMode, getOrchestrationMode } from './modes/shadow-mode.js';
import { getQueueStats } from './queue/queue-manager.js';
import { claimEventProcessing, clearProcessedMarker, isEventProcessed } from './events/replay-safety.js';
import { validateLineage, createChildEvent } from './events/lineage.js';
import { checkQueueHealth } from './observability/queue-monitor.js';
import { createEvent } from './events/event-schemas.js';
import { closeEventBus, publishTestEvent, subscribe } from './events/event-bus.js';
...
async function run() {
  ...
  test('isShadowMode() returns boolean', () => {
    return typeof isShadowMode() === 'boolean';
  });
  ...
  test('duplicate event_id is rejected', async () => {
    ...
    const first = await claimEventProcessing(testEventId);
    const second = await claimEventProcessing(testEventId);
    if (first) await clearProcessedMarker(testEventId);
  ...
  test('validateLineage detects invalid hop_count', () => {
    ...
    const result = validateLineage(badEvent as any);
    return !result.valid && result.errors.some(e => e.includes('negative'));
  });
  
  test('createChildEvent preserves correlation_id', () => {
    const parent = createEvent('plan.created', 'test', { plan_id: 'p1' }, { correlation_id: 'corr-123' });
    const child = createChildEvent(parent, 'step.completed', 'test', { step_id: 's1' });
